...
def main():
    ...
    ##checks to see if it can load a bg image, and if not it ignores it
    try:
        bgImage = tk.PhotoImage(file = "c:/Users/sofii/OneDrive/Desktop/bg.png")
        bgLabel = tk.Label(window, image = bgImage)
        bgLabel.place(relwidth = 1, relheight = 1, relx = 0, rely = 0)
    except tk.TclError as e:
        bgLabel = tk.Label(window, text = "No background image found", justify = "center", font = "Arial 12")
        bgLabel.place(relwidth = 1, relheight = 1, relx = 0, rely = 0)
    ...
    def ShowEntryList(entries):
        ...
        if imgReturn:
            ...
            returnButton.configure(text = altTextReturn, command = lambda: ReturnToAddPage(listWindow))
    ...
    amountFloat = tk.DoubleVar() #$amounts
    ...
    entryAmount = ttk.Entry(master = inputFrame, textvariable = amountFloat)
    ...
    def validateInput():
        try:
            ...
            # Validate category
            category = CategoryStr.get().lower()
            if category not in ["spending", "savings", "bills"]:
    ...
    if imgAdd:
        ...
        addButton.configure(text = altTextAdd, command = AddToList)
    ...
    if imgTotals:
        ...
        showListButton.configure(text = altTextTotals, command = lambda: ShowEntryList(entries))
    ...
    if imgExit:
        ...
        exitButton.configure(text = altTextExit, command = ButtonExit)
